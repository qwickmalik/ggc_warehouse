...
class UsersController extends AppController {
    ...
    var $name = 'User';
    var $uses = array('User', 'Usertype', 'Userdepartment', 'Setting', 'Currency', 'Eod', 'Eom', 
        'BalanceSheet', 'IncomeStatement', 'Equity', 'Zone', 'DailyDefault', 'Order', 'ClosingBalance');
    var $paginate = array(
...
//                
                ...
                $check = $this->Session->check('invent_acct');
                   if($check){
                    $this->Session->delete('invent_acct');
                }
                $check = $this->Session->check('sales_acct');
                   if($check){
                    $this->Session->delete('sales_acct');
                }
                if($result){
                ...
                $this->Session->write('warehouse_id', $result['Zone']['warehouse_id']);
...
//    }
    ...
    public function users() {
        ...
        $this->set('zones', $this->Zone->find('list',array('conditions' => array('Zone.deleted' => 0))));
//        pr($result);
//        exit;
             ...
             $result = $this->User->save($this->request->data);
                ...
                if ($result) {
                 ...
                 $message = 'User Profile Successfully Saved';
                    ...
                    $message = 'User Profile Saving Unsuccessful';
                    $this->Session->write('smsg', $message);
                    return "unsuccessful";
                }
            }
...
//                }
            ...
            if(!empty($password)){
                ...
                if ($result) {
                    ...
                    $message = 'User Profile Editing Unsuccessful';
                    $this->Session->write('smsg', $message);
                    return "unsuccessful";
                }
            }
            ...
            if(!is_null($userid) && !empty($userid)){
                 $this->set('users', $this->User->find('first',['conditions' => ['User.id' => $userid]]));
            }
        }
    }
...
//        return $eomCheck;
    ...
    function logout() {
        ...
        $this->Session->delete('shopPosttown');
        $this->Session->delete('shopPostCity');
        $this->Session->delete('shopTelephone');
        $this->Session->delete('shopMobile');
        $this->Session->delete('shopEmail');
        $this->Session->delete('accYear');
        $this->Session->delete('owner');
        $this->Session->delete('userInfo');
