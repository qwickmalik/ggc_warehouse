...
var sale = {
    ...
    discount_url: 'apply_discount',
    cart_url: 'check_cart',
    init: function() {
        // bind the button action to the apply discount link
        $("button#c_discount").click(function(event){
             
             sale.doDiscount();
             event.preventDefault();
        
        });
        // bind the open action to the add item link
        $("button#b_blue").click(function(event) {
            var tempsale = $("#TempSaleSellingPrice").val();
            var transact_mode = $("#TempSaleTransactionMode").val();
            ...
            if(quan == ""){
               ...
               $("#TempSaleQuantity").focus();
               $("#TempSaleQuantity").css("border-color", "red");
               return false;
            }
             $("#TempSaleQuantity").css("border-color", "dodgerblue");
            //alert(quan);
            var itemSer = $("#TempSaleItemId").val();
            itemSer = $.trim(itemSer);
            
            if(transact_mode == 'Sale' || transact_mode == 'Rental'){
            var stock = 0;
...
//            $(".large").css("border-color", "dodgerblue");
        ...
        $("button#c_blue").click(function(event) {
            ...
            salesperson = $.trim(salesperson);
             if(salesperson == ""){
                ...
                alert("Please click on new sale to refresh");
                return false;
            }
            ...
            //                 return false;
            //            }
            var com = confirm('Are you sure you wish to submit this sale?');
        if(!com){
            return false;
        }
            sale.doComplete();
    ...
    doDiscount: function(){
        ...
        $("#progress_msg").html("Processing discount....").show();
        $("#welcome_message").hide(5000);
        var discount_type = $("#TempSaleDiscountType").val();
        if(discount_type =="%"){
            if($("#SaleUseDiscountYes").is(':checked')) {
    
    var query = "action=apply_discount";
}else{
    alert("Discount won't be applied. Enter amount and continue.");
    return false;
}
        }else if(discount_type =="points"){
        var discount_use =  $("#SaleUseDiscount").val();
     
        var query = "action=apply_discount&discount_use=" + discount_use;
    }else{
         $("#progress_msg").hide();
                $("#welcome_message").show();
                $("#error_msg").html("Invalid Discount Type. Contact System Admin").show('slow');
                $("#welcome_message").hide(5000);
                alert("Invalid Discount Type. Contact System Admin");
        return false;
    }
        $("#error_msg").hide();
        $.ajax({
            url: this.discount_url,
            data: query,
            dataType: 'json',
            type: 'POST',
            success:function(data) {
           if(data['fail']){
               alert("Points entered are more than clients acquired points");
               return false;
           }
            sale.checkCart();
            },
    ...
    doComplete: function(){
        ...
        var hiddiscount = $("#hiddiscount").val();
        var client = $("#TempSaleClientId").val();
        var query = "action=complete&"+"hidtotcost=" + totcost + "&"+"hiddiscount=" + hiddiscount +  "&"+"client=" + client + "&" + $("#SaleIndexForm").serialize();
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                if( data['action'] == "receipt" ) {
                    ...
                    $("#progress_msg").hide();
                    $("#welcome_message").show();
                    $("#error_msg").html("Sorry No Credit Allowed!").show('slow');
                    $("#welcome_message").hide(7000);
                    alert("Sorry No Credit for Onetime Shoppers");
                    $("#SaleAmountPaid").focus().css("border-color", "red");
                    return false;
                }else if(data["feedback"] == 'overload'){
                var itemname = data["item"];
                    $("#progress_msg").hide();
                    $("#welcome_message").show();
                    $("#error_msg").html("Sorry,quantity selected for "+itemname+" is more than available.").show('slow');
                    $("#welcome_message").hide(7000);
                    alert("Sorry,quantity selected for "+itemname+" is more than available.");
//                    $("#SaleAmountPaid").focus().css("border-color", "red");
                    return false;
                }
    ...
    updatePrice: function(){
        ...
        $.post(url, query, function(data){
            if(data['status'] == 'ok'){
             ...
             $("#TempSaleWarehouseItemId").val(data['warehs']['id']);
            $("#TempSaleSellingPrice").val(data['salesResult']['selling_price']);
//            $(".sp_stock").html(data['warehs']['remaining_quantity']);
            ...
            $("#TempSaleWarehouseId").val(data['warehs']['warehouse_id']);
            $("#TempSaleTransactionMode").val(data['salesResult']['transaction_mode']);
            var mode = data['salesResult']['transaction_mode'];      
            var item_type = data['salesResult']['item_type'];
            var quantity = data['warehs']['remaining_quantity'];
            $("#stock").val(data['warehs']['remaining_quantity']);
                if(mode == 'Sale' || mode == 'Rental'){
                    $(".sp_stock").html(data['warehs']['remaining_quantity']);
                    $("#TempSaleQuantity").val(data['warehs']['remaining_quantity']);
           ...
           alert("Sorry Item details unavailable");
           return false;
        }
        },"json");
    }
...
var client = {
    ...
    init: function() {
        ...
        $("button#clientBtn").click(function(event) {
            var zone_id = $("#ClientZoneId").val();
            var name = $("#ClientClientName").val();
           
            if(name == ""){
                 alert("Please Enter a Client Name");
                $("#ClientClientName").css("border-color", "red");
                    $("#ClientClientName").focus();
                    return false;
            }
            
            
            $("#ClientClientName").css("border-color", "dodgerblue");
             if(zone_id == ""){
                 alert("Please Select a Zone");
                $("#ClientZoneId").css("border-color", "red");
                   $("#ClientZoneId").focus(); 
                    return false;
            }
             $("#ClientZoneId").css("border-color", "dodgerblue");
            client.doClient();
            $("#welcome_message").hide();
            event.preventDefault(event);
            return false;
...
//        });
        //callback function to handle user saving/submission
          $("button#editBtn").click(function(event) {
        ...
        $("button#userBtn").click(function(event) {
         ...
         if(userzone == ""){
                alert("Select user's branch/zone/sale-point!!");
                $("#UserZoneId").css("border-color", "red").focus();
                return false;  
            }
            $("#UserZoneId").css("border-color", "dodgerblue");
    ...
    userInfo: function(item){
        ...
        $.post(url, query, function(data){
            ...
            $("#UserUsertypeId").val(data['User']['usertype_id']);
              $("#UserZoneId").val(data['User']['zone_id']);    
        }, "json");
        return false;
    },
    ...
    editUser: function(){
        ...
        $("#progress_msg").html("Commiting Transaction").show();
        $("#welcome_message").hide(5000);
        var query = "action=editUser&" + $("#UserEditUserForm").serialize();
...
var discount_card = {
    ...
    init: function() {
        ...
        //callback function to handle supplier detail listing for editing
        $(".dcardAnchor a").click(function(event){
            var dcardID = $(this).attr("class");
            discount_card.discountInfo(dcardID);
            event.preventDefault();
            return false;
        });
    ...
    discountInfo: function(item){
           
        var url = $("#post_url").val();//'../Settings/discountInfo';
        var query = "action=discountInfo&" + "dcardID=" + item;
        $.post(url, query, function(data){
                 var status = data['DiscountCard']['status']
                 
            $("#DiscountCardDiscountCode").val(data['DiscountCard']['discount_code']);
            $("#DiscountCardPoints").val(data['DiscountCard']['points']);
            $("#DiscountCardDiscountcategoryId").val(data['DiscountCard']['discount_category_id']);
            if(status == 'active'){
                 $("#DiscountCardStatusActive").prop('checked',true);
            }
            else if(status == 'inactive'){
                $("#DiscountCardStatusInactive").prop('checked',true);
            }
            $("#DiscountCardId").val(data['DiscountCard']['id']);
...
var discount_category = {
    ...
    init: function() {
        ...
        //callback function to handle supplier detail listing for editing
        $(".dcatAnchor a").click(function(event){
            var dcatID = $(this).attr("class");
            discount_category.discountCatInfo(dcatID);
            event.preventDefault();
            return false;
        });
    ...
    discountCatInfo: function(item){
           
        var url = '../Settings/discountCatInfo';
        var query = "action=discountCatInfo&" + "dcatID=" + item;
        $.post(url, query, function(data){
                 var status = data['DiscountCategory']['category_type']
                 
            $("#DiscountCategoryCategoryName").val(data['DiscountCategory']['category_name']);
            $("#DiscountCategoryPercent").val(data['DiscountCategory']['percent']);
            
            if(status == 'Points'){
                 $("#DiscountCategoryCategoryTypePoints").prop('checked',true);
                 
            $("#DiscountCategoryMinUsablePoints").val(data['DiscountCategory']['min_usable_points']);
            $("#DiscountCategoryAmountPerPoint").val(data['DiscountCategory']['amount_per_point']);
            
                $(".point").show();
                 $(".percent").hide();
            }
            else if(status == 'Percentage'){
                $("#DiscountCategoryCategoryTypePercentage").prop('checked',true);
                
                $(".percent").show();
                $(".point").hide();
            }
            $("#DiscountCategoryId").val(data['DiscountCategory']['id']);
        ...
        // wrap it in a jQuery object and then invoke ajaxSubmit 
         var custPicture = $("#CustomerCustomerPhoto").val();
            ...
            if(custPicture == "" || custPicture == null){
                ...
                alert("Please Supply The Customer\'s Photo!!!");
                return false;
            }
//            
        $(this).ajaxSubmit(options); 
...
var fixedasset = {
    ...
    save_url: 'saveAsset',
    dispose_url: 'disposeAsset',

    init: function() {
        
        //callback function to handle asset info retrieval
        $(".assetAnchor a").click(function(event){
            var assetID = $(this).attr("class");
            fixedasset.assetInfo(assetID);
            event.preventDefault();
            return false;
        });
        //callback function to handle asset saving/submission
        $("button#save_dispose").click(function(event) {
            var assetname = $("#FixedAssetAssetName").val();
            var amount = $("#FixedAssetAmountPaid").val();
            assetname = $.trim(assetname);
            amount = $.trim(amount);
            
            if(assetname == ""){
                alert("Please Enter Asset Name/Description");
                $("#FixedAssetAssetName").css("border-color", "red");
                return false;
            }
            $("#FixedAssetAssetName").css("border-color", "dodgerblue");
            
            
            if(amount == ""){
                alert("Please Enter Disposal Amount");
                $("#FixedAssetAmountPaid").css("border-color", "red");
                return false;
            }
            $("#FixedAssetAmountPaid").css("border-color", "dodgerblue");
                
            
        });
        //callback function to handle asset saving/submission
        $("button#asset_save").click(function(event) {
            var assetname = $("#FixedAssetAssetName").val();
            var cost = $("#FixedAssetCost").val();
            var rate = $("#FixedAssetDepreciationRate").val(); 
            var user = $("#FixedAssetUserId").val();
         //  var begin_yes = $("#FixedAssetDepBeginYes").attr("checked");
          // var begin_no = $("#FixedAssetDepBeginNo").attr("checked");
//            var type = $("#FixedAssetAssetType").val();
            assetname = $.trim(assetname);
            cost = $.trim(cost);
            rate = $.trim(rate);
           // begin_yes = $.trim(begin_yes);
           // begin_no = $.trim(begin_no);
//            type = $.trim(type);
            //amount = $.trim(amount);
            user = $.trim(user);
            //remarks = $.trim(remarks);
...
//                $("#FixedAssetAssetType").css("border-color", "red");
//                return false;
//            }
//            $("#FixedAssetAssetType").css("border-color", "dodgerblue");
//            
//            
            
            if(cost == ""){
                alert("Please Enter Asset Cost");
                $("#FixedAssetCost").css("border-color", "red");
                return false;
            }
            $("#FixedAssetCost").css("border-color", "dodgerblue");
                
            
            
            if(rate == ""){
                alert("Please Enter Depreciation Rate");
                $("#FixedAssetDepreciationRate").css("border-color", "red");
                return false;
            }
            $("#FixedAssetDepreciationRate").css("border-color", "dodgerblue");
            
            if(user == ""){
                alert("Please Select User");
                $("#FixedAssetUserId").css("border-color", "red");
                return false;
            }
            $("#FixedAssetUserId").css("border-color", "dodgerblue");
            
//            if(begin_no != "checked" && begin_yes != "checked"){
//                alert("Please Select Depreciation Beginning");
//                $("#FixedAssetDepBeginYes").css("border-color", "red");
//                 $("#FixedAssetDepBeginNo").css("border-color", "red");
//                
//                return false;
//            }
//            $("#FixedAssetDepBeginYes").css("border-color", "dodgerblue");
//            $("#FixedAssetDepBeginNo").css("border-color", "dodgerblue");
        ...
        $('.button_del').click(function(event){
            ...
            var itemaction = $(this).attr('name');
            if(confirm("Are you sure you want to delete!!!")){
                fixedasset.deleteAsset(itemaction);
                $("#welcome_message2").hide();
            }
            event.preventDefault();
            return false;
    ...
    assetInfo: function(item){
          
        var url = '../FixedAssets/assetInfo';
        var query = "action=assetInfo&" + "assetId=" + item;
        $.post(url, query, function(data){
            var d = new Date(data['FixedAsset']['purchase_date']);
            var yr = d.getFullYear(); 
            var mn = d.getMonth(); 
            var dy =  d.getDate(); 
            $("#FixedAssetPurchaseDateDay").val(dy);
            $("#FixedAssetPurchaseDateMonth").val(mn);
            $("#is_year").val(yr);
            $("#FixedAssetDescription").val(data['FixedAsset']['description']);
            $("#FixedAssetAssetName").val(data['FixedAsset']['asset_name']);
            $("#FixedAssetCost").val(data['FixedAsset']['cost']);
            $("#FixedAssetAssetType").val(data['FixedAsset']['asset_type']);
            $("#FixedAssetDepBeginPurchaseDate").val(data['FixedAsset']['dep_begin']);
            $("#FixedAssetDepreciationRate").val(data['FixedAsset']['depreciation_rate']); 
            $("#FixedAssetUserId").val(data['FixedAsset']['user_id']);
            $("#FixedAssetId").val(data['FixedAsset']['id']);
    ...
    doAsset: function(){
        ...
        $("#progress_msg").html("Commiting Transaction").show();
        $("#welcome_message").hide(5000);
        var query = "action=saveAsset&" + $("#FixedAssetIndexForm").serialize();
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                if(  data['feedback'] == "unsuccessful") {                   
                    ...
                    $("#progress_msg").html("Expense Details Successfully Saved!!").show('slow').hide(5000);
                    $("#welcome_message").hide(5000);
                    $("#FixedAssetAssetName").val("");
                    $("#FixedAssetCost").val("");
                    $("#FixedAssetDepreciationRate").val("");
                    $("#FixedAssetUserId").val("");
                    $("#FixedAssetDescription").val("");
                    window.location.reload();
                    return false;
                }else if(data['feedback'] == "No Data"){
                    $("#progress_msg").hide();
                    ...
                    $("#error_msg").html("No Data. Server Not Receiving Data. Contact Admin!!").show('slow');
                    return false;
                }else if(data['feedback'] == "no dep"){
                    $("#progress_msg").hide();
                    $("#welcome_message").show();
                    alert("Depreciation Begins Not Selected!!");
                    return false;
                }
            },
            error: function() {
    ...
    disposeAsset: function(){
        ...
        $("#progress_msg").html("Commiting Transaction").show();
        $("#welcome_message").hide(5000);
        var query = "action=disposeAsset&" + $("#FixedAssetIndexForm").serialize();
      
        $("#error_msg").hide();
        $.ajax({
            url: this.dispose_url,
            data: query,
            dataType: 'json',
            type: 'POST',
            success:function(data) {
